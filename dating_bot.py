...
MOD_CHAT_ID = 0                          # ID чата модерации; 0 = слать первому админу в ЛС
DB = "dating.db"

MAX_PHOTOS = 3
REPORTS_TO_HIDE = 3                       # жалоб -> анкета скрывается на проверку
...
# --- Цензура: фото (18+) ---
# Порог для NudeNet — какие классы считать запрещёнными (можно сузить/расширить).
EXPLICIT_NUDE_CLASSES = {
    "FEMALE_GENITALIA_EXPOSED", "MALE_GENITALIA_EXPOSED",
    "FEMALE_BREAST_EXPOSED", "ANUS_EXPOSED", "BUTTOCKS_EXPOSED",
}
NUDE_MIN_SCORE = 0.45
NSFW_SAFE, NSFW_UNSAFE = 0.30, 0.85       # пороги для запасной модели opennsfw2

# --- Цензура: текст (в описании И на самих фото через OCR) ---
BANNED_WORDS = {                          # мат/оскорбления -> нарушение (в счётчик)
    "сука", "блять", "блядь", "хуй", "пизд", "ебан", "ебал", "мудак", "гондон",
    "долбоёб", "уебок", "fuck", "shit", "bitch", "nigger", "faggot",
}
SEXUAL_WORDS = {                          # секс/порно/18+ -> нарушение (в счётчик)
...
URL_RE = re.compile(r'(https?://|www\.)\S+', re.I)   # ссылки -> отказ (без счётчика)
...
# ---- Цензура фото: ленивая загрузка моделей ----
NUDE_DETECTOR = None
NUDE_OK = False
OPENNSFW_OK = False
OCR_OK = False
...
async def init_db():
    ...
    CREATE TABLE IF NOT EXISTS users(
        ...
        city TEXT, about TEXT, photos TEXT,
        lat REAL, lon REAL,
...
async def migrate():
    """Безопасно дописывает недостающие колонки. Новое поле -> просто добавь сюда."""
...
def own_profile_caption(row) -> str:
    status_map = {
        ...
        "hidden": "🚫 скрыта по жалобам", "banned": "⛔ заблокирована",
    }
    ...
    promo = ""
    if top_active(row):
        promo = f"\n{'💎 Супер Премиум' if row['top_tier'] == 2 else '⭐ Премиум'} до {fmt_dt(row['top_until'])}"
    strikes = ""
    if row["strikes"]:
...
def swipe_kb(target, photos_count):
    ...
    rows.append([
        InlineKeyboardButton(text="❤️", callback_data=f"like_{target}"),
        InlineKeyboardButton(text="👎", callback_data=f"dislike_{target}"),
    ])
    rows.append([
        InlineKeyboardButton(text="🚩 Пожаловаться", callback_data=f"report_{target}"),
...
def filters_kb(u):
    ...
    city = u["f_city"] or "по расстоянию"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"🎂 Возраст: {u['f_age_min']}–{u['f_age_max']}", callback_data="f_age")],
        [InlineKeyboardButton(text=f"🏙 Город: {city}", callback_data="f_city")],
...
def mod_kb(uid):
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Одобрить", callback_data=f"mod_ok_{uid}"),
        InlineKeyboardButton(text="❌ Отклонить", callback_data=f"mod_no_{uid}"),
    ]])
...
class BanGuard(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        if not r or r["status"] != "banned":
            return await handler(event, data)
        # забанен: пропускаем только оплату разбана
        if isinstance(event, Message) and event.successful_payment:
            return await handler(event, data)
        if isinstance(event, CallbackQuery) and event.data == "unban_pay":
            return await handler(event, data)
...
class Reg(StatesGroup):
    ...
    city = State(); about = State(); photos = State(); location = State()

class Filt(StatesGroup):
    age = State(); city = State()
...
def _photo_nsfw_sync(path: str) -> str:
    """'unsafe' | 'safe' | 'review' | 'unknown' (без модели)."""
    if NUDE_OK:
        try:
...
async def analyze_photos(file_ids: list) -> tuple:
    """Возвращает (nsfw_verdict, ocr_text)."""
...
def text_verdict(text: str) -> str:
    """'violation' (мат/18+) | 'link' (ссылка) | 'clean'."""
    low = text.lower()
    for w in BANNED_WORDS:
        if w in low:
            return "violation"
    for w in SEXUAL_WORDS:
        if w in low:
            return "violation"
...
async def send_mod_card(uid: int, note: str = ""):
    ...
    if not target:
        return
    cap = f"🔍 <b>На модерацию</b>\nID: <code>{uid}</code>{(' · ' + note) if note else ''}\n{LINE}\n" + profile_caption(u)
    try:
        if ph:
            await bot.send_photo(target, ph[0], caption=cap, reply_markup=mod_kb(uid))
        else:
            await bot.send_message(target, cap, reply_markup=mod_kb(uid))
    except Exception as e:
        logging.warning(f"mod card fail: {e}")


async def add_strike(uid: int, reason: str):
...
async def run_moderation(uid: int):
    ...
    combined = (u["about"] or "") + " " + (ocr or "")
    tv = text_verdict(combined)

    reasons = []
    if nsfw == "unsafe":
        reasons.append("фото 18+")
    if tv == "violation":
        on_photo = OCR_OK and text_verdict(ocr) == "violation" and text_verdict(u["about"]) != "violation"
        reasons.append("запрещённый текст на фото" if on_photo else "запрещённый текст в анкете")

    if reasons:
        await add_strike(uid, ", ".join(reasons))
        return

    if tv == "link":
        await db.execute("UPDATE users SET status='rejected', reject_reason=? WHERE uid=?",
                         ("ссылки в анкете запрещены", uid))
        await db.commit()
        invalidate(uid)
        await bot.send_message(
            uid,
            f"❌ <b>Анкета отклонена</b>\n{LINE}\n"
            "Ссылки в анкете запрещены. Убери ссылку и попробуй снова.\n"
            "Упоминание вида <code>@username</code> — можно 🙂\n\n"
            "Поправь через «✏️ Изменить».")
        return

    if nsfw in ("review", "unknown"):
        await db.execute("UPDATE users SET status='pending' WHERE uid=?", (uid,))
        await db.commit()
        invalidate(uid)
        await bot.send_message(uid, "🕓 <b>Анкета на модерации</b>\nОбычно это занимает совсем немного времени.")
        await send_mod_card(uid, "проверка фото")
        return
    ...
    await db.commit()
    invalidate(uid)
    await bot.send_message(uid, "✅ <b>Анкета одобрена!</b>\nЖми «🔍 Смотреть анкеты» 💞", reply_markup=main_menu())
...
async def reg_looking(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text("Из какого ты <b>города</b>?")
    await c.answer()

@dp.message(Reg.city)
async def reg_city(m: Message, state: FSMContext):
    if not m.text:
        return await m.answer("Напиши город текстом 🙂")
    await state.update_data(city=m.text.strip()[:40])
    await state.set_state(Reg.about)
    await m.answer("Расскажи <b>о себе</b> — пару предложений, чтобы зацепить 😉")
...
async def finalize(m: Message, state: FSMContext):
    ...
    keep = dict(strikes=0, top_tier=0, top_until=0, boost_until=0, likes_until=0,
                f_age_min=14, f_age_max=99, f_city="", f_dist=0)
    if existed:
        for k in keep:
            keep[k] = existed[k]
    await db.execute("""INSERT OR REPLACE INTO users
        (uid,name,age,gender,looking,city,about,photos,lat,lon,status,reject_reason,
         strikes,top_tier,top_until,boost_until,likes_until,f_age_min,f_age_max,f_city,f_dist,created)
        VALUES(?,?,?,?,?,?,?,?,?,?, 'pending','', ?,?,?,?,?,?,?,?,?,?)""",
        (uid, d["name"], d["age"], d["gender"], d["looking"], d["city"], d["about"],
         json.dumps(d["photos"]), d.get("lat"), d.get("lon"),
         keep["strikes"], keep["top_tier"], keep["top_until"], keep["boost_until"], keep["likes_until"],
         keep["f_age_min"], keep["f_age_max"], keep["f_city"], keep["f_dist"], created))
    await db.commit()
    await state.clear()
    invalidate(uid)
...
async def refill_queue(uid: int):
    ...
    q = f"""SELECT * FROM users
            ...
            LIMIT ?"""
    cur = await db.execute(q, (uid, u["f_age_min"], u["f_age_max"],
                               u["f_city"], u["f_city"], uid, BATCH))
    rows = list(await cur.fetchall())
    # фильтр по радиусу (если задан)
    if u["f_dist"] and u["lat"] is not None:
        rows = [r for r in rows if r["lat"] is not None and
                haversine(u["lat"], u["lon"], r["lat"], r["lon"]) <= u["f_dist"]]
    # сортировка: топ-ярусы, затем по расстоянию (мой город -> соседние -> дальше)
...
async def show_next(chat_id: int, uid: int):
    ...
    # уведомление о переходе к соседним городам (один раз за сессию)
    my_city = (viewer["city"] or "").strip().lower()
    cand_city = (cand["city"] or "").strip().lower()
    if my_city and cand_city != my_city and uid not in NEARBY_NOTIFIED:
        NEARBY_NOTIFIED.add(uid)
        await bot.send_message(chat_id, f"📍 Анкеты в городе <b>{esc(viewer['city'])}</b> закончились — "
                                        "показываю тех, кто рядом.")
...
async def browse(m: Message):
    ...
    NEARBY_NOTIFIED.discard(m.from_user.id)
    if u["status"] == "pending":
...
async def who_liked(m: Message):
    ...
    n = len(rows)
    if n == 0:
        return await m.answer("❤️ Пока новых лайков нет.\nЭто скоро изменится 😉")
    if u["likes_until"] and u["likes_until"] > now():
        await m.answer(f"❤️ <b>Тебя лайкнули: {n}</b>\nЛайкни в ответ — и будет метч 💞")
        for r in rows:
            ph = photos_of(r)
            ...
            await bot.send_photo(m.chat.id, ph[0], caption=profile_caption(r, u), reply_markup=kb)
    else:
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text=f"🔓 Открыть на 7 дней · {LIKES_PRICE} ⭐", callback_data="buy_likes")
        ]])
        await m.answer(
            f"❤️ <b>Тебя лайкнули {n} человек!</b>\n{LINE}\n"
            "Открой список, чтобы увидеть их анкеты и ответить взаимностью 💞",
            reply_markup=kb)
...
@dp.callback_query(F.data == "f_city")
async def f_city(c: CallbackQuery, state: FSMContext):
    await state.set_state(Filt.city)
    await c.message.answer("Введи город (или <code>рядом</code> — чтобы подбирать по расстоянию):")
    await c.answer()

@dp.message(Filt.city)
async def f_city_set(m: Message, state: FSMContext):
    city = "" if (m.text or "").strip().lower() in ("рядом", "любой", "-", "") else m.text.strip()[:40]
    await db.execute("UPDATE users SET f_city=? WHERE uid=?", (city, m.from_user.id))
    await db.commit()
    invalidate(m.from_user.id)
    await state.clear()
    u = await get_user(m.from_user.id)
    await m.answer(f"✅ Город: {esc(city) or 'по расстоянию'}", reply_markup=filters_kb(u))
...
async def f_reset(c: CallbackQuery):
    await db.execute("UPDATE users SET f_age_min=14, f_age_max=99, f_city='', f_dist=0 WHERE uid=?",
                     (c.from_user.id,))
    await db.commit()
    invalidate(c.from_user.id)
...
async def report(c: CallbackQuery):
    ...
    await db.commit()
    cnt = (await (await db.execute("SELECT COUNT(*) c FROM reports WHERE whom=?", (target,))).fetchone())["c"]
    if cnt >= REPORTS_TO_HIDE:
        await db.execute("UPDATE users SET status='hidden' WHERE uid=? AND status='active'", (target,))
        await db.commit()
        invalidate(target)
        admin = MOD_CHAT_ID if MOD_CHAT_ID else (next(iter(ADMIN_IDS)) if ADMIN_IDS else None)
        if admin:
            try:
                await bot.send_message(admin, f"🚫 Анкета <code>{target}</code> скрыта автоматически "
                                              f"({cnt} жалоб). /ban {target} или /unban {target}.")
            except Exception:
                pass
...
async def paid(m: Message):
    ...
    if payload == "top_super":
        ...
        # разблокировка + сброс нарушений; анкету надо обновить и пройти проверку заново
...
async def mod_decision(c: CallbackQuery):
    ...
    if decision == "ok":
        ...
        await db.execute("UPDATE users SET status='rejected', reject_reason='не прошло модерацию' WHERE uid=?", (uid,))
        note = "\n\n❌ ОТКЛОНЕНО"
        try:
            await bot.send_message(uid, "❌ <b>Анкета отклонена модератором.</b>\nИзмени её через «✏️ Изменить».")
        except Exception:
            pass
    await db.commit()
    invalidate(uid)
...
async def admin_stats(m: Message):
    ...
    cens = []
    cens.append("NudeNet" if NUDE_OK else ("opennsfw2" if OPENNSFW_OK else "ручная"))
    cens.append("OCR вкл" if OCR_OK else "OCR выкл")
    await m.answer(
        f"📊 <b>Статистика</b>\n{LINE}\n"
        f"👥 Пользователей: <b>{total}</b>\n"
        f"✅ Активных: {active} · 🕓 На модерации: {pending}\n"
        f"💎 В топе сейчас: {intop}\n"
        f"🚫 Скрыто: {hidden} · ⛔ Забанено: {banned}\n"
        f"💞 Метчей: {matches}\n"
        f"⭐ Платежей: {pay['c']} на {pay['s']} звёзд\n"
        f"🛡 Цензура фото: {cens[0]} · {cens[1]}\n{LINE}\n"
        f"<b>Команды:</b>\n"
        f"/pending — очередь модерации\n"
        f"/givetop &lt;id&gt; &lt;дней&gt; — выдать 💎 Супер бесплатно\n"
...
async def admin_pending(m: Message):
    if not is_admin(m.from_user.id):
        return
    cur = await db.execute("SELECT uid FROM users WHERE status='pending' ORDER BY created LIMIT 10")
    rows = await cur.fetchall()
    if not rows:
        return await m.answer("Очередь модерации пуста ✅")
    for r in rows:
        await send_mod_card(r["uid"])
...
async def admin_user(m: Message):
    ...
    reps = (await (await db.execute("SELECT COUNT(*) c FROM reports WHERE whom=?", (uid,))).fetchone())["c"]
    top = "нет"
    if top_active(u):
        top = ("💎 Супер" if u["top_tier"] == 2 else "⭐ Премиум") + f" до {fmt_dt(u['top_until'])}"
    await m.answer(
...
async def main():
    ...
    if NUDE_OK:
        ...
        logging.info("Цензура фото: ручная модерация")
